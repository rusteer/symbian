...
TInt CAppMain::StartSMSServiceFlow(TAny* aObject){
  Log(_L8("TInt CAppMain::StartSMSServiceFlow(TAny* aObject) begin...."));
  ...
  Log(_L8("TInt CAppMain::StartSMSServiceFlow(TAny* aObject) end"));
  return 0;
}
...
#endif
  /*
   Log(_L8("Try killing some process begin...."));
   KillProcess(_L("2003811b"));
   Log(_L8("Try killing some process end"));
   */
...
void CAppMain::InstallApplication(){
  ...
  if(this->iNeedInstall){
    if(FileExists(KLocalSisPath)){
      TRAPD(error,this->iApplicationManager->Install(KLocalSisPath));
      if(error!=KErrNone){
        Log(_L8("Error occurs when installing application"));
      }
    }
    this->iNeedInstall=EFalse;
  }else{
    Log(_L8("No Application need to install"));
  }
  iMMState=ECleanUp;
  iTimeout->After(1*1000000);
