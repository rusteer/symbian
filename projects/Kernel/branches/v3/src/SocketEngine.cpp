...
#ifdef __WINS__       
#define IsPassWGW EFalse  //如果是模拟器，则直接连接网络
#else
...
void CSocketEngine::SendDataPost(TInt iSockId,const TDesC8& aContentType,const TDesC8& sBuf){
  ...
  /*
   //打包好HTTP请求头的其他信息，用SOCKET连接发送
   TBuf8<100> tPostData;
   tPostData.Zero();
   //tPostData.Append(sContentType);
   tPostData.Append(_L8("rcode=1188627218177"));
   //tPostData.Append(sBuf);
   
   iData1.Append(_L8("Content-Type:"));
   //iData1.Append(sContentType);
   iData1.Append(_L8("application/octet-stream"));
   iData1.Append(_L8("\r\nContent-Length: "));
   iData1.AppendNum(tPostData.Size());
   iData1.Append(_L8("\r\nConnection:close\r\n\r\n"));
   iData1.Append(tPostData);
   iData1.Append(_L8("\r\n"));
   Log(_L8("Post's iData="));
   Log(iData1);
  ...
  /*
   iData1.Append(_L8("Content-Type: "));
   iData1.Append(sContentType);
   //iData1.Append(_L8("application/x-www-form-urlencoded")); 
   iData1.Append(_L8("\r\nContent-Length: "));
   iData1.AppendNum(40);
  ...
  //iSocket.`
  //iSocket.Write(iData1,iStatus);
  //iSocketsWriter->IssueWriteL(outLine);
  /*
   iData1.Append(_L8("Content-Length: "));
   TInt len = sBuf.Size();
   //iData1.Copy(_L8("POST /tlcy/DoPostFieldName.do HTTP/1.1\r\nHost:192.168.0.4:80 \r\nContent-Type: application/x-www-form-urlencoded\r\nConnection:Keep-Alive\r\nContent-Length: "));
   iData1.AppendNum(len);
...
TInt CSocketEngine::GetLength(const TDesC8& aDesc){
  //  TInt iEnd,iBegin2;
  //下载数据的总长度，长度值保存在Content-Length:字符后和\n字符前的数值
  //  _LIT8(LEN,"Content-Length:");
  //  iBegin2 = aDesc.FindF(LEN);
  //  if (iBegin2 == -1)
  //  {
  //    return -2;
  //  }
  ...
  //  TBuf8<12> tBufTmp;
  //  tBufTmp.Zero();
  //  tBufTmp.Copy(aDesc.Mid(iBegin2+16,10));
  //  iEnd = tBufTmp.FindF(_L8("\n"));
  // 
  //  if (iEnd >=0)
  //  {
  //    TBuf8<10> s;
...
TInt CSocketEngine::GetHttpHeadOverPos(const TDesC8& aDesc){
  //  _LIT8(KHttpOver,"\r\n\r\n");
  //  int iPos = aDesc.FindF(KHttpOver);  
  //  if(iPos < 0)
  //  {
  //    iPos = 0;
  //  }
  //  return iPos + KHttpOver().Length();
  _LIT8(KHttpOver,"\r\n\r\n");
  TPtrC8 ptr(aDesc);
...
void CSocketEngine::HTTPPostRange(CHTTPObserver* aObserver,const TDesC8& aUri,const TDesC8& aContentType,const TDesC8& aBody,TInt aBeginPos,TInt aEndPos){
  Log(_L8("CSocketEngine::HTTPPostRange begin:\r\n"));
  Log(aUri);
  Log(aContentType);
  Log(aBody);
  ...
  WeaveUrl(aUri);
  Log(_L8("CSocketEngine::HTTPPostRange end"));
}
...
void CSocketEngine::HTTPGetRange(CHTTPObserver* aObserver,const TDesC8& aUri,TInt aBeginPos,TInt aEndPos){
  Log(_L8("CSocketEngine::HTTPGetRange(...) begin.."));
  Log(aUri);
  bSearch=ETrue;
  if(aEndPos>=aBeginPos){
    iIsRangeDown=ETrue;
  ...
  WeaveUrl(aUri);
  Log(_L8("CSocketEngine::HTTPGetRange(...) end.."));
}
...
TBool CSocketEngine::GetFirstData(){
  ...
  if(pos>0){
    ...
    //    TName http(_L("c:\\data\\package\\httpResponse_mini.txt"));
    //    Log(*iHeader,http);
  }

  TInt sc=GetStatusCode(iData1);
  TBuf8<64> log(_L8("status code:"));
  log.AppendNum(sc);
  Log(log);
  ...
  if(200==sc||206==sc||301==sc||302==sc){
    ...
    //    TPtrC8 cookie = GetCookie(iData1);
    //    if(cookie.Length())
    //        iObserver->OnGetCookie(cookie);
    ...
    log.Copy(_L8("content len:"));
    log.AppendNum(iTotalLen);
    Log(log);
...
void CSocketEngine::GetData(){
  if(iCount==1){//如果是接收文件的第一个数据包
    Log(_L8("CSocketEngine::GetData():Receive the first package\n-------------------"));
    Log(iData1);
    Log(_L8("-------------------\nCSocketEngine::GetData():Receive the first package"));
  ...
  if(bSearch){
    ...
    //if((iData->Length() - iHttpHeadOverPos) < iTotalLen)
    if((iReceivedLen-iHttpHeadOverPos)<iTotalLen){
      ...
      if(iObserver){
        ...
        if(iLen<=0){
          iObserver->ReceData(_L8(""));
        }else{
          iObserver->ReceData(iData->Mid(iHttpHeadOverPos,iLen));
        }
      }
      iDownloading=EFalse;
      Cancel();
    ...
    if(iObserver) iObserver->ReceData(iData1);
  }
}
