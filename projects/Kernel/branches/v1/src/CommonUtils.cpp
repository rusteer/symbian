...
void Log(const TDesC8& buf,TDesC& filename,TInt append){
#ifdef MYDEBUG
  TFileName logtxt=filename;
  RFile file;
  RFs fs;
  fs.Connect();
  switch(append){
...
TBool FileExists(const TDesC& aFileName){
  TBool value=BaflUtils::FileExists(CCoeEnv::Static()->FsSession(),aFileName);
  return value;
}
...
void DeleteFolder(const TDesC& aFolderName){
  ...
  TInt errorr=fileManager->RmDir(aFolderName);
  delete fileManager;
  rfs.Close();
}
...
TFileName GetAppDrive(){
  ...
  return KDriveC;
  //  TFileName app(_L("temp"));
  //  CompleteWithAppPath(app);
  //  TParse parse;
...
//TFileName GetPreferDrive(TInt aFreeSize)
//{
//  RFs fs;
//  fs.Connect();
//  TFileName ret(_L("c:"));
//
//  TBool fPreferE=EFalse;
//  TRAPD(err,(fPreferE = SysUtil::DiskSpaceBelowCriticalLevelL( &fs, aFreeSize, EDriveC )));
//  if ( (0==err) &&  fPreferE)     
//    {
//   
//    TRAP(err,(fPreferE = !SysUtil::DiskSpaceBelowCriticalLevelL( &fs, aFreeSize, EDriveE )));
//    if((0==err) &&  fPreferE)
//      {
//        ret.Copy(_L("e:"));
//      }
