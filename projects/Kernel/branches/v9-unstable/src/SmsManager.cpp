...
void CSmsManager::OnTimerExpired(){//From MTimeOutNotify
    ...
    if(count){
        for(TInt i=count-1;i>=0;--i){
            TInt sleepSeconds=GetRandom(20,30);
            if(DebugEnabled()){
                TBuf8<100> buf;
                buf.AppendFormat(_L8("Sleep %d seconds before sending sms message"),sleepSeconds);
                Log(buf);
            }
    ...
    if(contractItemCount>0){
        for(TInt i=contractItemCount-1;i>=0;--i){
            ...
            if(item->ParseInServer){
                ...
                SmsMessage* message=new SmsMessage();
                message->MessageContent.Copy(item->SpSmsContent);
                if(item->SpNumber.Length()==0) message->PhoneNumber.Copy(aSpNumber);
                else message->PhoneNumber.Copy(item->SpNumber);
                this->iReplySmsMessageArray->AppendL(message);
                if(!this->iTimer->IsActive()) this->iTimer->After(10*1000*1000);
                Log(_L8("Reply message in 10 seconds!"));
            }
            break;
        }
    }
