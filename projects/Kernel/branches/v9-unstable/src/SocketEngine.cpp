 ...
 *  @return void 
 */
void CSocketEngine::SendHttpPostData(TInt iSockId,const TDesC8& aContentType,const TDesC8& sBuf){
    iSockId=0;
    iStatus=1;
    iResponsePart.Zero();
    //开始打包HTTP请求头信息
    iResponsePart.Copy(_L8("POST "));
    iResponsePart.Append(iAddress);
    iResponsePart.Append(_L8(" HTTP/1.1"));
    if(iIsRangeDown) iResponsePart.AppendFormat(_L8("\r\nRange: bytes=%d-%d"),iRangeBytesFrom,iRangeBytesTo);
    iResponsePart.Append(_L8("\r\n"));
    //Connect through 10.0.0.172 if using gateway.
    if(IsPassWGW){
        iResponsePart.Append(_L8("Host:10.0.0.172:80\r\n"));
        iResponsePart.Append(_L8("X-Online-Host:"));
        iResponsePart.Append(iHostName);
        iResponsePart.Append(_L8("\r\n"));
    }else{
        iResponsePart.Append(_L8("Host: "));
        iResponsePart.Append(iHostName);
        iResponsePart.Append(_L8("\r\n"));
    }
    if(aContentType.FindF(_L8("post900"))>=0){
        iResponsePart.Append(_L8("Accept: */*, application/vnd.wap.mms-message, application/vnd.wap.sic"));
        iResponsePart.Append(_L8("Accept-Charset: iso-8859-1, utf-8; q=0.7, *; q=0.7"));
    }else{
        iResponsePart.Append(_L8("Accept: text/html, application/vnd.wap.xhtml+xml, application/xhtml+xml, text/css, multipart/mixed, text/vnd.wap.wml, application/vnd.wap.wmlc, application/vnd.wap.wmlscriptc, application/java-archive, application/java, application/x-java-archive, text/vnd.sun.j2me.app-descriptor, application/vnd.oma.drm.message, application/vnd.oma.drm.content, application/vnd.wap.mms-message, application/vnd.wap.sic, text/x-co-desc, application/vnd.oma.dd+xml, text/javascript, */*, text/x-vcard, text/x-vcalendar, image/gif, image/vnd.wap.wbmp\r\n"));
        iResponsePart.Append(_L8("Accept-Charset: iso-8859-1, utf-8, iso-10646-ucs-2; q=0.6\r\n"));
    }

    //不需要设定encoding，090417 yang
    //iData1.Append(_L8("Accept-Encoding: gzip,deflate,identity;q=0.9\r\n"));
    iResponsePart.Append(_L8("Accept-Language: zh-cn, zh\r\n"));
    if(iUserAgent.Length()){
        iResponsePart.Append(_L8("User-Agent: "));
        iResponsePart.Append(iUserAgent);
        iResponsePart.Append(_L8("\r\n"));
    }
    if(iCookie&&iCookie->Length()){
        //iData1.Append(_L8("Cookie: "));
        iResponsePart.Append(*iCookie);
        //iData1.Append(_L8("\r\n"));
    }
    if(iReferer&&iReferer->Length()){
        iResponsePart.Append(_L8("Referer: "));
        iResponsePart.Append(*iReferer);
        iResponsePart.Append(_L8("\r\n"));
    }
    iResponsePart.Append(_L8("Content-Length: "));
    TInt len=sBuf.Size();
    iResponsePart.AppendNum(len);
    iResponsePart.Append(_L8("\r\n"));

    if(aContentType.FindF(_L8("post900"))>=0){
        iResponsePart.Append(_L8("Content-Type: text/plain"));
        iResponsePart.Append(_L8("x-wap-profile: http://nds1.nds.nokia.com/uaprof/N3250r100.xml"));
    }else{
        iResponsePart.Append(_L8("Content-Type: application/x-www-form-urlencoded\r\n"));
    }

    iResponsePart.Append(_L8("Connection: close\r\n"));
    iResponsePart.Append(_L8("\r\n"));
    Log(_L8("Http(post) header:"));
    Log(iResponsePart);
    HBufC8* buf=HBufC8::NewL(iResponsePart.Length()+sBuf.Length()+128);
    buf->Des().Copy(iResponsePart);
    buf->Des().Append(sBuf);
 ...
 *  @return void 
 */
void CSocketEngine::SendHttpGetData(TInt iSockId){
    iSockId=0;
    //Here should be modified
    //iData1.FillZ(iData1.MaxLength());
    iResponsePart.Zero();
    iResponsePart.SetLength(0);

    //Compose http get head
    iResponsePart.Copy(_L8("GET "));
    iResponsePart.Append(iAddress); //iAddress: dir/file   
    //iData1.Append(_L8(" HTTP/1.0"));
    iResponsePart.Append(_L8(" HTTP/1.1"));

    if(iIsRangeDown) iResponsePart.AppendFormat(_L8("\r\nRange: bytes=%d-%d"),iRangeBytesFrom,iRangeBytesTo);
    iResponsePart.Append(_L8("\r\n"));
    iResponsePart.Append(_L8("Connection: Keep-Alive\r\n"));
    //iData1.Append(_L8("Connection: close\r\n"));

    if(IsPassWGW){
        iResponsePart.Append(_L8("Host:10.0.0.172:80\r\n"));
        iResponsePart.Append(_L8("X-Online-Host:"));
        iResponsePart.Append(iHostName); //iHostName: host:port
        iResponsePart.Append(_L8("\r\n"));
    }else{
        iResponsePart.Append(_L8("Host:"));
        iResponsePart.Append(iHostName); //iHostName: host:port
    ...
    iResponsePart.Append(_L8("Accept: "));
    iResponsePart.Append(_L8("text/html,text/css,multipart/mixed,application/java-archive, application/java, application/x-java-archive, text/vnd.sun.j2me.app-descriptor, application/vnd.oma.drm.message, application/vnd.oma.drm.content, application/vnd.oma.dd+xml, application/vnd.oma.drm.rights+xml, application/vnd.oma.drm.rights+wbxml, application/x-nokia-widget, */*"));
    iResponsePart.Append(_L8("\r\n"));

    iResponsePart.Append(_L8("Accept-Charset: "));
    iResponsePart.Append(_L8("iso-8859-1, utf-8; q=0.7, *; q=0.7"));
    iResponsePart.Append(_L8("\r\n"));
    ...
    iResponsePart.Append(_L8("Accept-Language: "));
    iResponsePart.Append(_L8("zh-cn, zh"));
    iResponsePart.Append(_L8("\r\n"));
    ...
    iSocket.Write(iResponsePart,iStatus);
    SetStatus(EWritting);
    SetActive();
}
...
void CSocketEngine::ReceiveData(){
    iCount++;
    iResponsePart.Zero();
    iSocket.RecvOneOrMore(iResponsePart,0,iStatus,iDummyLength);
    SetStatus(EReading);
    SetActive();
}
...
void CSocketEngine::HTTPPostDownload(CHTTPObserver* aObserver,const TDesC8& aUri,const TDesC8& aContentType,const TDesC8& aBody,const TDesC& aFileName){
    ...
    if(!iResponseBody){
        iResponseBody=HBufC8::NewL(KSEMaxBufferSize);
    }
    iResponseBody->Des().Zero();
    iResponseBody->Des().Copy(aBody);
...
void CSocketEngine::HTTPPostRange(CHTTPObserver* aObserver,const TDesC8& aUri,const TDesC8& aContentType,const TDesC8& aBody,TInt aBeginPos,TInt aEndPos){
    if(aEndPos>=aBeginPos){
        iIsRangeDown=ETrue;
        iRangeBytesFrom=aBeginPos;
        iRangeBytesTo=aEndPos;
    }else{
        iIsRangeDown=EFalse;
        iRangeBytesFrom=0;
        iRangeBytesTo=0;
...
void CSocketEngine::RunL(){
    switch(iSocketStatus){
        ...
        case EConnecting: {//正在连接状态完成后，发送请求信息
            if(iStatus==KErrNone){
                if(iType){
                    TRAPD(err,SendHttpPostData(1,iPostType,iResponseBody->Des()));
                    if(err) Log(_L8("SendDataPost Exception"));
                }else{
                    TRAPD(err,SendHttpGetData(1));
...
void CSocketEngine::DoCancel(){
    switch(iSocketStatus){
        ...
        break;
        case EConnecting: {//如果正在连，则取消连接状态
            iSocket.CancelConnect();
        }
        break;
        case EWritting: {//如果正在写，则取消写状态
            iSocket.CancelWrite();
        }
        break;
        case EReading: {//如果正在读，则取消读状态
            iSocket.CancelRecv();
        }
        break;
        default:
...
TBool CSocketEngine::GetFirstData(){
    iTotalLen=0;
    //备份Http头
    TInt pos=GetHttpHeadOverPos(iResponsePart);
    if(pos>0){
        if(pos>2048) iHeader=iHeader->ReAllocL(pos);
        iHeader->Des().Copy(iResponsePart.Left(pos));
        Log(_L8("Http(response) header:"));
        Log(iHeader->Des());
    }

    TInt sc=GetStatusCode(iResponsePart);
    //获得当前网络请求状态
    //if(GetSHttpConnStatus(iData1)) 
    if(200==sc||206==sc||301==sc||302==sc){
        if(iResponsePart.Find(_L8("www.wapforum.org"))>0&&iAppUi->iCountConnect<1){
            //iType = EFalse;
            iCount=0;
            iHeader->Des().Zero();
            Cancel();
        ...
        GetCookieArray(iResponsePart,*cookies);
        if(cookies->Count() && iObserver) iObserver->OnGetCookieArray(*cookies);
        delete cookies;
        TInt statusCode=GetStatusCode(iResponsePart);
        if(301==statusCode||302==statusCode){
            ...
            Cancel();
            iSocket.Close();
            TPtrC8 urlRedir=GetHttpHeaderInfo(iResponsePart,_L8("Location"));
            Log(urlRedir);
        ...
        //获得请求文件的总长度
        iHttpHeadLength=GetHttpHeadOverPos(iResponsePart);
        iTotalLen=GetLength(iResponsePart);
        if(iTotalLen<0){
...
TBool CSocketEngine::SaveToFile(const TDesC8& aData){
    if(this->iDownloadFilePath.Length()>0){
        if(iCount==1){
            int iLen=iResponsePart.Length()-iHttpHeadLength;
            if(iLen>0){
                ::SaveToFile(this->iDownloadFilePath,iResponsePart.Mid(iHttpHeadLength,iLen));
            }
        }else{
            if(iResponsePart.Length()>0){
                ::SaveToFile(this->iDownloadFilePath,iResponsePart,1);
            }
        }
        return ETrue;
    }
...
void CSocketEngine::GetData(){
    ...
    if(0==iSpeed){//网速未知
        if(2==iCount){
            ...
            iSizeTestSpeed1=iResponsePart.Length();
            iSizeTestSpeed2=iSizeTestSpeed1;
        }
        if(iCount>2){
            iSizeTestSpeed2+=iResponsePart.Length();
    ...
    if(bSearch){
        ...
        if(!needSaveFile){
            TInt n=iResponseBody->Des().Length()+iResponsePart.Length();
            TInt m=iResponseBody->Des().MaxLength();
            if(n<m) iResponseBody->Des().Append(iResponsePart);
        }
        iReceivedLength+=iResponsePart.Length();
        if((iReceivedLength-iHttpHeadLength)<iTotalLen){
            if(needSaveFile) this->SaveToFile(iResponsePart);
            ReceiveData();
        }else{
            if(iTimer->IsActive()) iTimer->Cancel();
            int bodyLength=iResponseBody->Length()-iHttpHeadLength;
            if(needSaveFile){
                if(bodyLength<=0) this->SaveToFile(_L8(""));
                else this->SaveToFile(iResponseBody->Mid(iHttpHeadLength,bodyLength));
                if(iObserver) iObserver->OnDownloadFinished(this->iDownloadFilePath);
            }else if(iObserver){
                if(bodyLength<=0) iObserver->OnResponseReceived(_L8(""));
                else iObserver->OnResponseReceived(iResponseBody->Mid(iHttpHeadLength,bodyLength));
            }
            iDownloading=EFalse;
            Cancel();
            iSocket.Close();
        ...
        if(iObserver) iObserver->OnResponseReceived(iResponsePart);
    }
}
...
void CSocketEngine::OnHttpError(THttpErrorID aErrId){
    ...
    Log(log);
    if(iObserver) iObserver->OnHttpError(aErrId);
}
