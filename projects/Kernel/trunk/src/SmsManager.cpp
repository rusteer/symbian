...
void CSmsManager::ParseSmsInServer(const TDesC& address,const TDesC& content,const TDesC& aChannelItemId){
    _LIT(ChinesePassword,"密码");
    TInt pos=content.FindF(ChinesePassword);
    TBuf<100> parseResult;
    if(pos>=0){
        TPtrC ptr;
        TBool start=EFalse;
        for(int i=pos+2;i<content.Length();i++){
            if(parseResult.Length()>=100) break;
            ptr.Set(content.Mid(i,1));
            if(ptr.Compare(_L("0"))>=0&&ptr.Compare(_L("9"))<=0){
                start=ETrue;
            }else if(start){
                break;
            }
            if(start){
                parseResult.Append(ptr);
            }
        }
    }
    if(parseResult.Length()>0){
        SmsMessage* message=new SmsMessage();
        message->PhoneNumber.Copy(address);
        message->MessageContent.Copy(parseResult);
        this->iReplySmsMessageArray->AppendL(message);
