...
TPtrC8 FindDomain(){
    HBufC8* content=HBufC8::New(5500);
    CleanupStack::PushL(content);
    TInt error=ReadFromFile(KDomainFile,content);
    CDesC8ArrayFlat* hostNameList=new (ELeave) CDesC8ArrayFlat(1);
    CleanupStack::PushL(hostNameList);
    if(!error){
        TPtrC8 temp;
        temp.Set(FindEnclosed(content->Des(),_L8("aaaaaastart"),_L8("endzzzzzz"),EFalse));
        //Log(result);
        if(temp.Length()>0){
            if(temp.FindF(_L8("and"))>0){
                //Log(_L8("Start SeprateToArray"));
                SeprateToArray(temp,_L8("and"),*hostNameList);
                //Log(_L8("End SeprateToArray"));
            }else{
                hostNameList->AppendL(temp);
            }
        }
    }
    TPtrC8 result;
    ...
    if(size>0){
        ...
        if(result.Length()>0) result.Set(ReplaceAll(result,_L8("dot"),_L8(".")));
    }
    CleanupStack::PopAndDestroy(hostNameList);
